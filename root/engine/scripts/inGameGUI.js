...
function GUI(textureCanvas) {
    ...
    textureCanvas.width = MathUtils.GetGreatestDouble(maxWidth);
    textureCanvas.height = MathUtils.GetGreatestDouble(msgHeight);
    // These need to happen again now that the canvas has changed
    TwoD.SetStyles(textSize);
    // Draw text
    var offset = (textureCanvas.height - msgHeight) * 0.5;
    for (var i = 0; i < outText.length; i++) {
        var textY = (i + 1) * textSize + offset;
        TwoD.DrawText(outText[i], textureCanvas.width / 2, textY);
    }
}